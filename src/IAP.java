...
public class IAP extends CordovaPlugin {
    ...
    protected static final String TAG = "VINOOS";
                ...
                if (result.isFailure()) {
                    ...
                    if (response == IabHelper.IABHELPER_BAD_RESPONSE || response == IabHelper.IABHELPER_UNKNOWN_ERROR) {
                    	...
                    	if (!mSubscribe) {
                    		...
                    		getSubscribeProduct(sku, callbackContext);                    		
                    	}
                    } else {
    ...
    private void getSubscribeProduct(final String productId, final CallbackContext callbackContext) {
                ...
                try {
                    if (OwnedSkus != null) {
                    	for (int i=0; i < OwnedSkus.size() ; i++) { 
                    		Purchase purchase = OwnedSkus.get(i);
                    		if (new String(purchase.getSku()).equals(productId)) {
                        		JSONObject pluginResponse = new JSONObject();
                                pluginResponse.put("orderId", purchase.getOrderId());
                                pluginResponse.put("packageName", purchase.getPackageName());
                                pluginResponse.put("productId", purchase.getSku());
